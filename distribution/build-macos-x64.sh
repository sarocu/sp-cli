...
copyDarwinDirectory(){
  createInstallationDirectory
  cp -r macos ${TARGET_DIRECTORY}/
  chmod -R 755 ${TARGET_DIRECTORY}/macos/artifacts
  chmod -R 755 ${TARGET_DIRECTORY}/macos/resources
  chmod 755 ${TARGET_DIRECTORY}/macos/distribution.xml
}

copyBuildDirectory() {
    sed -i '' -e 's/__VERSION__/'${VERSION}'/g' ${TARGET_DIRECTORY}/macos/scripts/postinstall
    sed -i '' -e 's/__PRODUCT__/'${PRODUCT}'/g' ${TARGET_DIRECTORY}/macos/scripts/postinstall
    chmod -R 755 ${TARGET_DIRECTORY}/macos/scripts/postinstall

    sed -i '' -e 's/__VERSION__/'${VERSION}'/g' ${TARGET_DIRECTORY}/macos/distribution.xml
    sed -i '' -e 's/__PRODUCT__/'${PRODUCT}'/g' ${TARGET_DIRECTORY}/macos/distribution.xml
    chmod -R 755 ${TARGET_DIRECTORY}/macos/distribution.xml

    sed -i '' -e 's/__VERSION__/'${VERSION}'/g' ${TARGET_DIRECTORY}/macos/resources/*.html
    sed -i '' -e 's/__PRODUCT__/'${PRODUCT}'/g' ${TARGET_DIRECTORY}/macos/resources/*.html
    chmod -R 755 ${TARGET_DIRECTORY}/macos/resources/
    ...
    cp -a ./macos/artifacts/sp ${TARGET_DIRECTORY}/darwinpkg/Library/${PRODUCT}/${VERSION}
...
function buildPackage() {
    ...
    --version ${VERSION} \
    --scripts ${TARGET_DIRECTORY}/macos/scripts \
...
function buildProduct() {
    ...
    productbuild --distribution ${TARGET_DIRECTORY}/macos/distribution.xml \
    --resources ${TARGET_DIRECTORY}/macos/resources \
...
function createInstaller() {
    ...
    buildPackage
    buildProduct ${PRODUCT}-darwin-installer-x64-${VERSION}.pkg
    while true; do
...
function createUninstaller(){
    cp macos/resources/uninstall.sh ${TARGET_DIRECTORY}/darwinpkg/Library/${PRODUCT}/${VERSION}
